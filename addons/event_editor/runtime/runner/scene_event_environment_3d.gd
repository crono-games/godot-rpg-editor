class_name SceneEventEnvironment3D
	...
	while not stack.is_empty():
		...
		if node.is_in_group("EventInstance"):
			...
			if event_id != "":
				...
				push_warning("SceneEventEnvironment3D: event_instance without id skipped: %s" % [node.name])
			if node.name != "":
...
func get_player(player_group: String = "player") -> Node:
	...
	while not stack.is_empty():
		var node = stack.pop_back()
		if node is Node3D and node.is_in_group(player_group):
...
func set_event_graphics(event_id: String, graphics: Dictionary) -> void:
	...
	if event.has_node("Sprite3D"):
		sprite_node = event.get_node("Sprite3D")
	...
	if sprite_node == null:
		return
	if texture_path == "":
		if sprite_node is Sprite3D:
			(sprite_node as Sprite3D).texture = null
			(sprite_node as Sprite3D).visible = false
	...
	if tex is Texture2D:
		if sprite_node is Sprite3D:
			(sprite_node as Sprite3D).texture = tex
			(sprite_node as Sprite3D).visible = true
			(sprite_node as Sprite3D).hframes = hframes
			(sprite_node as Sprite3D).vframes = vframes
			(sprite_node as Sprite3D).frame = frame
			if (sprite_node as Sprite3D).has_method("set_offset"):
...
func set_event_position(event_id: String, position: Vector3) -> bool:
	...
	if event is Node2D:
		...
		return true
	if event is Node3D:
		(event as Node3D).position = position
		return true
	return false

func _to_2d_position(position: Vector3) -> Vector2:
	# 2D workflows store Y in z (x, 0, y). Keep backward compatibility with older (x, y, 0).
	var y2d := position.z if absf(position.z) > 0.0001 else position.y
	return Vector2(position.x, y2d)
...
func set_event_animation_timing(event_id: String, anim_step_time: float, max_cycles_per_step: float) -> void:
	...
	if event == null:
		return
	if event.has_method("set"):
		event.set("anim_step_time", maxf(0.01, anim_step_time))
